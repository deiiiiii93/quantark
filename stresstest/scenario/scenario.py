...
class Stress:
    ...
    def __post_init__(self):
        ...
        if self.level == StressLevel.UNDERLYING and not self.target:
            raise ValidationError("Target underlying is required for UNDERLYING level stress")
        
        if self.level == StressLevel.POSITION and not self.target:
            raise ValidationError("Target position_id is required for POSITION level stress")
        
        if self.level == StressLevel.PORTFOLIO and self.target:
            raise ValidationError("Target should not be specified for PORTFOLIO level stress")
