...
class ParameterChange:
    ...
    def __post_init__(self):
        ...
        valid_params = equity_params + rate_params
...
class DayStep:
    ...
    def __post_init__(self):
        ...
        if self.day_index < 0:
            raise ValidationError(
                f"day_index must be non-negative, got {self.day_index}"
            )
    ...
    def get_changes_for_underlying(self, underlying: str) -> List[ParameterChange]:
        ...
        return [
            ...
            if c.level == StressLevel.PORTFOLIO
            or (c.level == StressLevel.UNDERLYING and c.target == underlying)
        ]
...
class DayPath:
    ...
    def get_summary(self) -> str:
        ...
        lines.append(
            f"Parameters changed: {', '.join(sorted(self.get_all_parameters()))}"
        )
        ...
        for step in self.steps:
            ...
            label = step.label or ""
            changes_str = (
                ", ".join(str(c) for c in step.changes)
                if step.changes
                else "no changes"
            )
