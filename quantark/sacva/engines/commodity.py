"""Commodity risk engine for SA-CVA (MAR50.74-50.77). Single factor per bucket."""
...
class CommodityEngine(BaseRiskClassEngine):
    """Commodity delta and vega risk (MAR50.74-50.77)."""
    ...
    def risk_weight(self, s: CVASensitivity, risk_type: RiskType) -> float:
        return SP.commodity_vega_rw(s.bucket) if risk_type == RiskType.VEGA \
            else SP.commodity_delta_rw(s.bucket)
