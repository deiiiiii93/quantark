...
- **Follow-ups landed 2026-07-24:** both actionable follow-ups above are now shipped. (a) The deprecated `EngineParams.bump_size=1e-4` shim is retired: default-constructed params flow `BumpConfig`'s documented 1% spot bump to every bump-greek path (base-engine BUMP greeks, `GreeksCalculator.calculate_numerical_delta/gamma` defaults, legacy `EquityPosition.get_trade_greeks` — which now also draws its vol/rate bumps from the per-factor config instead of reusing the spot bump); explicit `bump_size=` still works but emits a `DeprecationWarning`. Gates in `test/test_greeks_bump_config.py::TestBumpSizeShimRetirement`. (b) The schedule-based Snowball/Phoenix vol MC engines (LV/Heston/SLV/SLV-QE) accept opt-in `substeps_per_interval=n` (mirroring `HestonDCNMCEngine`): each contractual interval is refined into n equal SDE steps while recorded nodes, payoff kernels, event stats, and RQMC batching stay on the contractual grid (the Sobol session dimension scales by n). Default n=1 is bitwise-identical to the pre-feature engines; `substeps_per_interval=n` is bitwise-identical to driving the same engine with the n-refined dt array and subsampling every n-th column. Gates in `test/test_mc_substeps_per_interval.py`.
