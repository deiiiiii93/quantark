...
- **Date:** 2026-07-23
- **Status:** Investigation complete; root cause verified empirically (forced-ownership sign reversal, projection collapse, damping-only inertness). Phase-1 discrete-event projection shipped opt-in as `PDEParams(event_projection="cell_average")` with `quantark/asset/equity/engine/pde/event_projection.py`; gates in `test/test_pde_event_projection.py`. Default remains `"nodal"` pending repricing review. Per-event damping strengthening and the default flip are follow-ups.
