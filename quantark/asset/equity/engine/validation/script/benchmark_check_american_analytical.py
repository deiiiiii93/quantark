"""
Benchmark Check Script for American Option Analytical Engine
...
from quantark.util.enum.engine_enums import AmericanAnalyticalMethod, MonteCarloMethod
...
class BenchmarkResults:
    ...
    def summary(self):
        ...
        print(f"{'='*90}")
        print(f"{'Case':<40} {'Analytical':>12} {'MC':>12} {'SE':>10} {'Error':>10} {'Status':>8}")
        print(f"{'-'*90}")
...
def run_benchmark_tests(results: BenchmarkResults):
    ...
    for method in (
        AmericanAnalyticalMethod.BS93,
        AmericanAnalyticalMethod.BS02,
        AmericanAnalyticalMethod.BAW,
    ):
        analytical_engine = AmericanOptionAnalyticalEngine(method=method)
        for case in test_cases:
            ...
            results.add_result(f"{method.name} {name}", analytical_price, mc_price, mc_se)
