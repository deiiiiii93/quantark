...
class ReportGenerator:
    ...
    def generate_html_report(
        self, filename: Optional[str] = None, include_plots: bool = True
    ) -> str:
        """
        ...
        if filename is None:
            ...
            filename = (
                f"backtest_report_{self.results.config.underlying}_{timestamp}.html"
            )
...
"""
    ...
    def _generate_metrics_section(self, metrics: dict) -> str:
        ...
        # Check if this is a Fixed Income backtest
        is_fi = "dv01_tracking_error" in metrics or "avg_abs_dv01" in metrics
        ...
        if is_fi:
            hedge_metrics = {
                ...
                "DV01 Tracking Error": f"${metrics.get('dv01_tracking_error', 0):,.0f}",
                "Avg Absolute DV01": f"${metrics.get('avg_abs_dv01', 0):,.0f}",
                "Max DV01 Exposure": f"${metrics.get('max_dv01_exposure', 0):,.0f}",
                "DV01 Hedge Effectiveness": f"{metrics.get('dv01_hedge_effectiveness', 0):.2%}",
            }
        else:
            hedge_metrics = {
                "Number of Hedges": f"{metrics.get('num_hedges', 0)}",
        ...
        # Add FI-specific risk metrics if available
        if is_fi:
            if "avg_duration" in metrics:
                risk_metrics["Avg Duration (years)"] = (
                    f"{metrics.get('avg_duration', 0):.2f}"
                )
            if "avg_convexity" in metrics:
                risk_metrics["Avg Convexity"] = (
                    f"{metrics.get('avg_convexity', 0):,.0f}"
                )
        ...
        hedging_title = "DV01 Hedging Metrics" if is_fi else "Hedging Metrics"

        return f"""
        <div class="section">
            <h2>Performance Metrics</h2>
            {create_metrics_table("P&L Metrics", pnl_metrics)}
            {create_metrics_table(hedging_title, hedge_metrics)}
    ...
    def generate_text_report(self, filename: Optional[str] = None) -> str:
        ...
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = (
                f"backtest_report_{self.results.config.underlying}_{timestamp}.txt"
            )
        ...
        lines.append(
            f"Period: {self.results.config.start_date.date()} to {self.results.config.end_date.date()}"
        )
        ...
        lines.append(
            f"Transaction Costs:   ${summary.get('total_transaction_costs', 0):>12,.2f}"
        )
        lines.append("")
        ...
        lines.append(
            f"Max Drawdown:        {metrics.get('max_drawdown_pct', 0):>12.2f}%"
        )
        ...
        # Check if FI backtest
        is_fi = "dv01_tracking_error" in metrics or "avg_abs_dv01" in metrics

        lines.append("-" * 80)
        if is_fi:
            lines.append("DV01 HEDGING METRICS")
            lines.append("-" * 80)
            lines.append(
                f"Hedge Frequency:     {metrics.get('hedge_frequency', 0):>12.3f} per day"
            )
            lines.append(
                f"Avg Hedge Cost:      ${metrics.get('avg_hedge_cost', 0):>12.2f}"
            )
            lines.append(
                f"DV01 Tracking Err:   ${metrics.get('dv01_tracking_error', 0):>12,.0f}"
            )
            lines.append(
                f"Avg Abs DV01:        ${metrics.get('avg_abs_dv01', 0):>12,.0f}"
            )
            lines.append(
                f"Max DV01 Exposure:   ${metrics.get('max_dv01_exposure', 0):>12,.0f}"
            )
            lines.append(
                f"Hedge Effectiveness: {metrics.get('dv01_hedge_effectiveness', 0):>12.2%}"
            )
        else:
            ...
            lines.append(
                f"Delta Tracking Err:  {metrics.get('delta_tracking_error', 0):>12.4f}"
            )
            lines.append(
                f"Avg Abs Delta:       {metrics.get('avg_abs_delta', 0):>12.2f}"
            )
        lines.append("")

        # Add FI risk metrics if applicable
        if is_fi:
            lines.append("-" * 80)
            lines.append("FIXED INCOME RISK METRICS")
            lines.append("-" * 80)
            if "avg_duration" in metrics:
                lines.append(
                    f"Avg Duration:        {metrics.get('avg_duration', 0):>12.2f} years"
                )
            if "avg_convexity" in metrics:
                lines.append(
                    f"Avg Convexity:       {metrics.get('avg_convexity', 0):>12,.0f}"
                )
            lines.append("")
