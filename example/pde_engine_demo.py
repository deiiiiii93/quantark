...
from asset.equity.engine import PDEEngine, BlackScholesEngine
...
from util.enum.engine_enums import PDEMethod, EngineType, AmericanAnalyticalMethod
...
def demo_pde_engine_basics():
    ...
    print(f"Grid: {pde_engine.params.grid_size} space steps, {pde_engine.params.time_steps} time steps")
...
def demo_greeks_calculation():
    ...
    greeks_pde = calculator.calculate_numerical_greeks(amer_put, pricing_env, pde_engine)
    ...
    greeks_barrier = calculator.calculate_numerical_greeks(barrier, pricing_env, pde_engine)
...
def demo_pde_vs_analytical():
    ...
    print(f"Relative Error:             {abs(price_bs - price_pde) / price_bs * 100:.4f}%")
    ...
    print(f"Relative Error:                   {abs(price_analytical - price_pde) / price_analytical * 100:.4f}%")
...
def demo_solver_caching():
    ...
    for strike in strikes:
        option = EuropeanVanillaOption(strike=strike, option_type=OptionType.CALL, maturity=1.0)
        price = pde_engine.price(option, pricing_env)
        print(f"  K={strike:3d}: ${price:.6f}")
    
    print(f"\nSolver cache size: {len(pde_engine._solver_cache)}")
    print(f"Cached solver types: {[t.__name__ for t in pde_engine._solver_cache.keys()]}")
...
def main():
    ...
    print("\n" + "█" * 80)
    print("█" + " " * 78 + "█")
    print("█" + "  PDEEngine Demonstration - Unified PDE Pricing Interface".center(78) + "█")
    print("█" + " " * 78 + "█")
    print("█" * 80)
