...
Factory functions for creating pre-configured Snowball option structures with sensible defaults.
...
The system SHALL provide a `create_airbag_snowball()` function that creates a snowball with reduced participation below airbag barrier.

#### Scenario: Create airbag snowball with default airbag barrier
Given initial_price=100, strike=100, maturity=1.0
When create_airbag_snowball(initial_price, strike, maturity) is called
Then a SnowballOption is returned with:
  - airbag_barrier = 60.0 (60% of initial_price)
  - participation_rate = 0.5 (50% participation below airbag)
