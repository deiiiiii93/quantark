...
## Purpose
Provides PDE-based (partial differential equation) pricing engines for convertible bonds. These engines implement the Bloomberg Jump-Diffusion model and the Tsiveriotis-Fernandes decomposition model. PDE methods offer high accuracy, efficient Greeks computation via finite differences on the solution grid, and natural handling of continuous exercise boundaries.
