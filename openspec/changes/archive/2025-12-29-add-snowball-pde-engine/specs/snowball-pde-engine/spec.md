...
Provides a PDE pricing engine for Snowball (autocallable) options using a Two-Surface method. This enables deterministic pricing with stable Greeks calculation as an alternative to Monte Carlo simulation.

## Related Capabilities

- `equity-pde-engine`: Unified PDE engine interface (extends to support SnowballOption)
- `snowball-option-helpers`: Snowball product factory functions (compatible with PDE engine)
