...
def test_grid_and_alignment_agree_on_ki_nodes():
    ...
    If ``_ki_monitor_times`` drops the KI monitor times but ``_build_grids``
    ...
    for knocked_in in (False, True):
        ...
        monitor_times = solver._ki_monitor_times(product, tau)
        nodes_in_grid = solver._ki_nodes_in_grid(product)
        assert bool(monitor_times) == nodes_in_grid, (
            f"knocked_in={knocked_in}: grid has "
            f"{len(monitor_times)} KI monitor times but the alignment "
