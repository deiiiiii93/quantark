"""Smoke test for the phoenix demo study over the real PDE and QUAD engines.

Like the snowball study test, this asserts *soundness, not outcome*: at the
tiny sampling budget used here the benchmark cannot meet its standard-error
budget, so INCONCLUSIVE is the correct result. What must hold is that every
arm runs, produces finite numbers, and gets gated.
...
def certified(tmp_path_factory, small_study):
    out = tmp_path_factory.mktemp("phoenix")
...
def test_the_memory_case_actually_flips_the_payoff(study):
    """A case that changed no payoff would be five wasted minutes of sampling."""
    memory_case = next(c for c in study.cases if c.name == "memory")
    assert memory_case.product_params == {"memory_coupon": True}

    quad = next(c for c in study.candidates if c.name() == "equity.phoenix.quad")
    plain = quad.evaluate(CaseSpec(name="ordinary")).values["pv"]
    with_memory = quad.evaluate(memory_case).values["pv"]
    # Memory coupons only ever add value back, so the sign is knowable a priori.
...
def test_report_is_written(certified):
    ...
    assert "phoenix-flat-bsm" in report
    for candidate in CANDIDATES:
        assert candidate in report


def test_pde_records_the_grid_its_profile_resolves_to(study):
    pde = next(c for c in study.candidates if c.name() == "equity.phoenix.pde")
    params = pde.params()
