...
def main():
    ...
    # Long call spread
    long_call = Position(
        product=EuropeanVanillaOption(strike=400.0, option_type=OptionType.CALL, maturity=1.0),
        quantity=50,
        entry_price=20.0,
    ...
    short_call = Position(
        product=EuropeanVanillaOption(strike=420.0, option_type=OptionType.CALL, maturity=1.0),
        quantity=-50,
        entry_price=12.0,
        underlying=underlying,
        engine=engine,
    ...
    long_put = Position(
        product=EuropeanVanillaOption(strike=380.0, option_type=OptionType.PUT, maturity=1.0),
        quantity=30,
        entry_price=15.0,
        underlying=underlying,
        engine=engine,
    ...
    print(f"Proportional rate: {cost_model.proportional_rate:.4f} ({cost_model.proportional_rate * 10000:.1f} bps)")
    print(f"Slippage: {cost_model.slippage_coefficient:.6f} ({cost_model.slippage_type})")
    ...
    print(f"  Cost as % of P&L:    {(summary['total_transaction_costs'] / abs(summary['total_pnl']) * 100) if summary['total_pnl'] != 0 else 0:.2f}%")
    ...
    dashboard = InteractiveDashboard(results, save_dir="plots/advanced_example/interactive")
    ...
    print(f"  Final P&L:           ${summary['total_pnl']:,.2f} ({summary['total_return_pct']:.2f}%)")
    print(f"  Sharpe Ratio:        {metrics['sharpe_ratio']:.3f}")
    print(f"  Max Drawdown:        {metrics['max_drawdown_pct']:.2f}%")
    print(f"  Transaction Costs:   ${summary['total_transaction_costs']:,.2f}")
    print()
