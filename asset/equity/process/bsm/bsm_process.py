...
class BSMProcess:
    ...
    def __post_init__(self):
        ...
        if self.dividend_yield < 0:
            raise ValidationError(f"Dividend yield must be non-negative, got {self.dividend_yield}")
        # Allow negative rates
        if self.risk_free_rate < -0.10 or self.risk_free_rate > 0.50:
            raise ValidationError(f"Risk-free rate outside reasonable bounds: {self.risk_free_rate}")
    ...
    def __repr__(self):
        return (f"BSMProcess(S={self.spot:.2f}, σ={self.volatility:.2%}, "
