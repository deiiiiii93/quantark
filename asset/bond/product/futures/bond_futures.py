...
class BondFutures(BaseBondForward):
    ...
    def validate(self) -> None:
        ...
        if self.tick_size <= 0:
            raise ValidationError(
                f"Tick size must be positive, got {self.tick_size}"
            )
    ...
    def calculate_conversion_factor(self, bond: FixedBond) -> float:
        ...
        years_to_maturity = (
            bond.maturity_date - self.delivery_date
        ).days / 365.0
    ...
    def calculate_implied_repo_rate(
        ...
        """
        if self.futures_price is None:
            raise ValidationError("Futures price must be set for implied repo calculation")
    ...
    def __repr__(self):
        ...
        return (f"BondFutures(delivery={self.delivery_date.date()}, "
