...
from asset.bond.product.couponbond.frn import FloatingRateBond, FloatingCashFlow
...
class FRNDiscountEngine:
    ...
    def discount_margin(
        ...
        for iteration in range(max_iterations):
            ...
            current_price = self.price(
                frn, valuation_date, settlement_date,
                spread_adjustment=dm
            )
    ...
    def dirty_price_from_yield(
        ...
        return self.price_from_yield(
            frn, ytm, valuation_date, settlement_date, 
            clean_price=False, assumed_index_rate=assumed_index_rate
        )
    ...
    def clean_price_from_yield(
        ...
        return self.price_from_yield(
            frn, ytm, valuation_date, settlement_date, 
            clean_price=True, assumed_index_rate=assumed_index_rate
        )
    ...
    def full_analysis(
        ...
        if market_price is not None:
            ...
            sm = self.simple_margin(
                frn, market_price, valuation_date, clean_price
            )
            
            try:
                ytm = self.yield_to_maturity(
                    frn, market_price, valuation_date, settlement_date, 
                    clean_price, assumed_index_rate
                )
