...
class NotionalQuantityPolicy(Enum):
    """Policy for reconciling notional and quantity when both are provided.

    When both notional and quantity are provided to an option constructor,
    they should satisfy: notional ≈ quantity × initial_price. This policy
    determines how to handle cases where they are inconsistent.

    The relationship represents dual representations of position size:
    - notional: Total dollar value of the position (e.g., $1,000,000)
    - quantity: Number of contracts or shares
    - initial_price: Reference price for conversion
    """

    STRICT = "strict"
    """Raise ValidationError if both provided and inconsistent.

    Use this mode when you want explicit validation to catch data errors.
    The error message includes both the provided values and expected values.
    """

    PREFER_NOTIONAL = "prefer_notional"
    """Derive quantity from notional when both provided and inconsistent.

    This is the default policy. It treats notional as the source of truth,
    which aligns with business convention for structured products (Snowball,
    Phoenix) which are typically quoted in notional amounts.

    Formula: quantity = notional / initial_price
    """

    PREFER_QUANTITY = "prefer_quantity"
    """Derive notional from quantity when both provided and inconsistent.

    Use this mode when quantity is the primary input and notional should
    be derived from it.

    Formula: notional = quantity × initial_price
    """
